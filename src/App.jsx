...
// ================== FAST MOBILE TAP ACTIVATION ==================
// Some mobile browsers delay the synthetic "click" generated after a touch
// gesture. This hook keeps the animated swipe, but activates real buttons and
// links immediately on a clean touch pointer-up. A later duplicate browser
// click is suppressed.
function useFastTouchActivation() {
  useEffect(() => {
    if (typeof document === "undefined") return undefined;

    const style = document.createElement("style");
    style.dataset.fastTouchActivation = "true";
    style.textContent = `
      button,
      a[href],
      [role="button"] {
    ...
    const onPointerUp = (event) => {
      ...
      const endTarget = findInteractive(event.target);
      if (!endTarget || endTarget !== start.target) return;

      const dx = event.clientX - start.x;
      const dy = event.clientY - start.y;

      // A real tap, not a drag that started on a control.
      if (Math.hypot(dx, dy) > 12) return;

      // Prevent the browser from scheduling its own delayed compatibility click.
      if (event.cancelable) event.preventDefault();

      lastFastTarget = endTarget;
      lastFastActivationAt = performance.now();
    ...
    const onClickCapture = (event) => {
      if (dispatchingImmediateClick) return;
      if (!lastFastTarget) return;
      if (performance.now() - lastFastActivationAt > 900) return;

      const clickedTarget = findInteractive(event.target);
      if (clickedTarget !== lastFastTarget) return;

      // Suppress only the duplicate delayed click generated by the browser.
...
const PRODUCTS = [
  {
    ...
    externalUrl: "/The_Water_Spirit_by_Leo_Tolstoy.zip",
