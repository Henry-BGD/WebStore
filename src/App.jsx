...
function useSwipeTabs({
  ...
  // Pointer capture is useful during a swipe, but it must always be released.
  // Otherwise some mobile browsers can keep routing the next tap to <main>.
  const capturedPointerIdRef = useRef(null);
  const capturedTargetRef = useRef(null);

  // Suppress only the synthetic click generated by the swipe itself.
  // The timestamp prevents the flag from remaining active indefinitely.
  ...
  const shouldIgnoreTarget = useCallback((target) => {
    try {
      return !!target?.closest?.(
        'a, button, input, textarea, select, [role="slider"], input[type="range"], [data-no-swipe="true"]'
      );
    } catch {
      return false;
    }
  ...
  const releaseCapturedPointer = useCallback((event) => {
    const target = event?.currentTarget || capturedTargetRef.current;
    const pointerId = event?.pointerId ?? capturedPointerIdRef.current;

    if (target && pointerId != null) {
      try {
        const hasCapture = target.hasPointerCapture?.(pointerId);
        if (hasCapture !== false) target.releasePointerCapture?.(pointerId);
      } catch {}
    }
  ...
  const resetGesture = useCallback(
    (event, { clearClickSuppression = true } = {}) => {
      ...
      if (clearClickSuppression) suppressClickUntilRef.current = 0;

      setDragX(0);
      setIsDragging(false);
    },
    [releaseCapturedPointer, cancelPendingFrame]
  );
  ...
  const onPointerDown = useCallback(
    (e) => {
      ...
      // Buttons and links should behave like buttons and links, not begin a tab swipe.
      if (shouldIgnoreTarget(e.target)) return;

      // A new physical touch means this is not the synthetic click from the old swipe.
  ...
  const onPointerMove = useCallback(
    (e) => {
      ...
      if (axisLock.current === "y") {
        // Release capture immediately so normal vertical scrolling and taps stay responsive.
        resetGesture(e);
        return;
      }
      ...
      // Elastic drag effect. CSS touch-action: pan-y already handles browser gestures,
      // so preventDefault() is intentionally not used here.
      const damp = 0.85;
      setDragXRaf(dx * damp);
    },
    [enabled, lockPx, restraintPx, resetGesture, setDragXRaf]
  );
  ...
  const onPointerUp = useCallback(
    (e) => {
      ...
      const madeRealSwipe = wasTracking && wasHorizontal && Math.abs(dx) >= thresholdPx;

      // Release pointer capture before changing tabs. This is the important part
      // for avoiding the temporary "dead" buttons on mobile browsers.
      ...
      if (!enabled || !madeRealSwipe) {
        suppressClickUntilRef.current = 0;
        return;
      }

      // Block only the ghost click belonging to this swipe. If the browser does not
      // generate one, the suppression expires automatically after a short interval.
      suppressClickUntilRef.current = performance.now() + 350;

      if (dx > 0) onPrev?.();
      else onNext?.();
    },
    [enabled, onPrev, onNext, thresholdPx, cancelPendingFrame, releaseCapturedPointer]
  );
  ...
  const onClickCapture = useCallback((e) => {
    ...
    // Suppress only the click automatically emitted after the swipe.
    e.preventDefault?.();
    e.stopPropagation?.();
    suppressClickUntilRef.current = 0;
  }, []);
